...
const getDealers = async () => {
  ...
  const fileName = 'tata-commercial-dealers.csv';
  fs.writeFileSync(fileName, headers, (err) => {if (err) throw err;});
  const apiUrl = 'http://dealerlocator.tatamotors.com:8080/tmdealerlocator/json/budealers?buType=TRUCKS%2FBUSES&locType=Vehicle+Showroom&';
  let dealerIds = [];
  let count = 0;
